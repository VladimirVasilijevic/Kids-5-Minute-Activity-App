...
async function createUser(email, password, role = USER_ROLES.FREE_USER) {
  try {
    const { auth, firestore, FieldValue, displayName } = initializeFirebase('prod');
    ...
    printMessage(`🚀 Creating user in ${displayName} environment...`, 'warning');
    printMessage(`⚠️  WARNING: This will create a user in PRODUCTION!`, 'warning');
...
if (!email || !password) {
  printMessage('Usage: node prod/create-user.js <email> <password> [role]', 'info');
  printMessage(`Roles: ${getAvailableRoles()}`, 'info');
  printMessage('Example: node prod/create-user.js test@example.com password123 admin', 'info');
  printMessage('\n⚠️  WARNING: This will create a user in PRODUCTION!', 'warning');
  process.exit(1);
}
