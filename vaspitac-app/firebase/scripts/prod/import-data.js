...
async function importData(importType) {
  try {
    const { firestore, FieldValue, displayName } = initializeFirebase('prod');
    ...
    printMessage(`🚀 Starting data import in ${displayName} environment...`, 'warning');
    printMessage(`⚠️  WARNING: This will import data to PRODUCTION!`, 'warning');
...
if (!importType) {
  printMessage('Usage: node prod/import-data.js <type>', 'info');
  printMessage(`Types: ${getAvailableImportTypes()}`, 'info');
  printMessage('Examples:', 'info');
  printMessage('  node prod/import-data.js activities', 'info');
  printMessage('  node prod/import-data.js blog', 'info');
  printMessage('  node prod/import-data.js categories', 'info');
  printMessage('  node prod/import-data.js about', 'info');
  printMessage('  node prod/import-data.js all', 'info');
  printMessage('\n⚠️  WARNING: This will import data to PRODUCTION!', 'warning');
  process.exit(1);
}
