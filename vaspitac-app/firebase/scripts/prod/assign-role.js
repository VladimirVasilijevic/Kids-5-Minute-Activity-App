...
async function assignRole(userId, newRole) {
  try {
    ...
    printMessage(`🚀 Assigning role in ${displayName} environment...`, 'warning');
    printMessage(`⚠️  WARNING: This will modify a user in PRODUCTION!`, 'warning');
...
if (!userId || !newRole) {
  printMessage('Usage: node prod/assign-role.js <userId> <role>', 'info');
  printMessage(`Roles: ${getAvailableRoles()}`, 'info');
  printMessage('Example: node prod/assign-role.js abc123 admin', 'info');
  printMessage('\n⚠️  WARNING: This will modify a user in PRODUCTION!', 'warning');
  process.exit(1);
}
